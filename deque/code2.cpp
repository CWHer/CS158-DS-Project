...
bool isEqual(Ans &ans, Test &test) {
    // std::cout<<ans.size()<<' '<<test.size()<<std::endl;
...
bool insertTest() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        switch (randnum() % 2) {
        // switch(1){
            case 0: ans.insert(ans.begin() + pos, x);
                    deq.insert(deq.begin() + pos, x);
                    
                    break;
            case 1: ans.insert(ans.end() - pos, x);
