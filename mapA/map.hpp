...
namespace sjtu
{
    ...
    private:
        ...
        {
            NodeType node_type;
            int tree_size, height;
            Node *child[2], *father;
            value_type *storage;

            Node(NodeType type, int height, Node *fa = nullptr)
                : node_type(type), height(height), father(fa)
            {
                tree_size = 1;
                child[0] = child[1] = nullptr;
                storage = nullptr;
            }
            Node(NodeType type, int height, Node *fa, const value_type &value)
                : node_type(type), height(height), father(fa)
            {
                tree_size = 1;
                child[0] = child[1] = nullptr;
            ...
            void pushUp()
            {
                ...
                for (int i = 0; i < 2; ++i)
                    if (child[i] != nullptr)
                    {
                        ...
                        if (child[i]->height + 1 > height)
                            height = child[i]->height + 1;
                    }
            }
        };
        Node *root;
        ...
        // >>>>>> utilities
        int height(Node *x) { return x == nullptr ? 0 : x->height; }
        ...
        pair<Node *, bool> insert(Node *&o, Node *fa, const value_type &value)
        {
            if (o == nullptr)
            {
                o = new Node(NodeType::Data, 1, fa, value);
            ...
            if (height(o->child[k]) - height(o->child[k ^ 1]) == 2)
            {
                Node *&child = o->child[k];
                int child_k = height(child->child[0]) < height(child->child[1]);
                if (k ^ child_k)
                    rotate(child, child_k);
                rotate(o, k);
            }
            o->pushUp();
            return ret;
        ...
        bool balance(Node *&o, int k)
        {
            if (height(o->child[k]) - height(o->child[k ^ 1]) == 1)
                return false;
            if (height(o->child[k]) == height(o->child[k ^ 1]))
                return true;

            Node *&child = o->child[k];
            int child_k = height(child->child[0]) < height(child->child[1]);
            if (height(child->child[0]) != height(child->child[1]) && (k ^ child_k))
            {
                rotate(child, child_k);
                rotate(o, k);
                return true;
            ...
            rotate(o, k);
            return height(o->child[0]) == height(o->child[1]);
        }

        // (need_balance, is_removed)
        pair<bool, bool> remove(Node *&o, Node *fa, const value_type &value)
        {
            if (o == nullptr)
            ...
            if (equal(o->storage, value.first))
            {
                ...
                // swap o and succ(o)
                //  then, remove o recursively
                Node *x = succ(o);
                Node *child = x->child[1];

                x->father->child[x->father->child[1] == x] = o;
                x->father = fa;

                for (int i = 0; i < 2; ++i)
                    if (o->child[i] != nullptr)
                        x->child[i] = o->child[i], o->child[i]->father = x;
                o->child[1] = child, o->child[0] = nullptr;
                o = x; // change o (fa->child)

                pair<bool, bool> ret = remove(o->child[1], o, value);
                o->pushUp();
                return !ret.first ? ret : pair<bool, bool>(balance(o, 0), true);
            }
            NodeType type = o->node_type;
            int k = type;
            if (type == NodeType::Data)
                k = compare_func(o->storage->first, value.first);
            pair<bool, bool> ret = remove(o->child[k], o, value);
            o->pushUp();
            return !ret.first ? ret : pair<bool, bool>(balance(o, k ^ 1), true);
        }
        // <<<<<< utilities
    ...
    public:
        ...
        map()
        {
            head = root = new Node(NodeType::Head, 1);
            tail = root->child[1] = new Node(NodeType::Tail, 1, root);
            root->pushUp();
        }
        map(const map &other)
        {
        ...
        void erase(iterator pos)
        {
            ...
            if (pos.node_ptr->node_type != NodeType::Data)
                throw invalid_iterator();
            if (!remove(root, nullptr, *(pos.node_ptr->storage)).second)
                throw invalid_iterator();
        }
        /**
